...
Bu klasör **yalnız Android** projesidir. iOS / App Store build **yok**.

- Store build: `npm run build:play:android` (`eas.json` profil `play-store`, `--platform android`, AAB)
- Preview APK: `npm run build:preview:android` — test binary değil; mağaza testi Play Internal AAB
- Submit: `npm run submit:play:android`
- **Onaysız `eas build` yok.** Kullanıcı açıkça `build:play:android` / Submit demeden başlatma.
- iOS build bu klasörden çalıştırılmaz. App Store IPA için `../ios` kullanılır.
...
Expo SDK **56** + Expo Router · Auth/Data/Actions Context · Ödeme: web Stripe (login’li `/plans` CTA; uygulama içi IAP yok)
