 ...
 * 7. Play UX: ML Kit barcode activity PORTRAIT kilidi kaldırılır (Android 16 büyük ekran).
 * 8. Display cutout: ALWAYS (Android 15 SHORT_EDGES/DEFAULT deprecated).
 *    Opt-out edge-to-edge YOK — Android 16’da çalışmaz; RN StatusBar bytecode uyarısı
 *    Expo/RN çekirdeğinden gelir, uygulama setStatusBarColor çağırmaz.
 */
...
const MLKIT_BARCODE_ACTIVITIES = [
  'com.google.mlkit.vision.codescanner.internal.GmsBarcodeScanningDelegateActivity',
  'com.google.mlkit.vision.codescanner.internal.GmsBarcodeScanningSelfDelegateActivity',
];
...
function isMainActivity(name) {
  const n = String(name || '');
  return n === '.MainActivity' || n.endsWith('.MainActivity') || n.endsWith('.ReactActivity');
}

function patchMainActivityCutout(application) {
  application.activity = application.activity || [];
  for (const act of application.activity) {
    if (!isMainActivity(act.$?.['android:name'])) continue;
    act.$['android:windowLayoutInDisplayCutoutMode'] = 'always';
    const replace = String(act.$['tools:replace'] || '')
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean);
    if (!replace.includes('android:windowLayoutInDisplayCutoutMode')) {
      replace.push('android:windowLayoutInDisplayCutoutMode');
    }
    act.$['tools:replace'] = replace.join(',');
  }
}

function overrideMlKitOrientation(application) {
  application.activity = application.activity || [];
  for (const name of MLKIT_BARCODE_ACTIVITIES) {
    const existing = application.activity.find((a) => a.$?.['android:name'] === name);
    const attrs = {
      'android:name': name,
...
function withDailyForegroundService(config) {
  ...
  return withAndroidManifest(config, (mod) => {
    ...
    if (application) {
      ...
      patchMainActivityCutout(application);
      overrideMlKitOrientation(application);
    }
    return mod;
  });
}
