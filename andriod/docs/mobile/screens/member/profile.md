...
4b. **Kan tahlilleri** `HealthLabFilesPanel` — yalnız `bloodWorkUploadIntent === 'yes'`. Üye yükler / açar / siler (`health-lab-results` + `members.data.healthTest.bloodWorkFiles`). Hayır / daha sonra / sorulmadı → panel yok.
...
7. Üyelik Planınız card — `migrateLegacyToPackages` + `isPackageEntryActive`; multi-package chip picker; remaining days / one-time doctor session counter (`isOneTimePlan`, `countUsedDoctorSessions`, `getRemainingDays`)
   - Expiring copy (Android): `Bu paketin süresi yakında doluyor — kesintisiz devam için Planlar’dan yenileyin. İptal için Ödeme Yönetimi.`
   - Expiring copy (iOS): `Bu paketin süresi yakında doluyor.`
   - Expired copy (Android): `Bu paketin süresi doldu — yenilemek için Planlar sayfasını kullanın.`
   - Expired copy (iOS): `Bu paketin süresi doldu.`
   - **Ödeme Yönetimi** → `/(member)/profile/payments` — **yalnız Android**. **iOS:** gizli (ileride).
   - **Planları karşılaştır / paket ekle** → **Android** web `/plans` handoff (`openWebCheckoutHandoff`). **iOS:** yok.
   - Ücretsiz “Premium özellikler için plan yükseltin” → Android ödemeler. iOS: yükseltme linki yok; “Ücretsiz plandasınız.”
...
Fields: photo, name, email (**read-only**), phone (**read-only / locked after registration**), city, district (Turkey city/district sheets), **Şifre değiştir** (`PasswordChangeSection` — mevcut + yeni + tekrar; `POST /api/auth` `{ action: "password-change" }`). Sosyal-only hesapta şifre formu yok.
...
- [ ] Kan tahlili paneli yalnız “Evet” intent; yükle / aç / sil
- [ ] Profili Düzenle içinde şifre değiştir (mevcut + yeni + tekrar)
