...
**OEM arka plan (Android):** Samsung / Xiaomi / Huawei “Arka planda otomatik kapat” / agresif pil kısıtı FCM teslimini keser. `androidBackgroundDelivery.ts` izin sonrası ayar ekranı prompt’u gösterir. Testte bu kısıt kapatılmalı; aksi halde uygulama kapalıyken sohbet/randevu push gelmez, yerel DATE habit zilleri yine çalışır.

**Kanal kayıt (2026-08-28):** Android `yeniform-alerts-v3` / `-silent` auth’tan önce `app/_layout` açılışında kaydedilir. `ensureNotificationChannel` hata verirse `channelReady` sıfırlanır ve sonraki çağrı yeniden dener. Kanal yokken gelen FCM “Miscellaneous” kanalına düşer veya sessiz kalır.

**OEM pil (2026-08-28):** Samsung / Xiaomi / Huawei “arka planda otomatik kapat” FCM teslimini keser. İzin `granted` ise bir kez Alert → uygulama ayarları (`expo-intent-launcher` `APPLICATION_DETAILS_SETTINGS`). iOS’ta kanal ve OEM öldürme yok.
