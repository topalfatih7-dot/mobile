...
- else: `${amount} tekrar`
- with sets (üye Programs / Calendar): ``${sets} set × ${amountText}`` when `sets` is present
- Coach cart payload includes `sets` (default 3) via `createCartEntry` / `hydrateDayCartsFromEntries`
