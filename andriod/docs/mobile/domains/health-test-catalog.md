...
**Scores (YeniForm Sağlık Skoru — 2 aşama):** Core complete → manuel Analizi Başlat (`analysisStage: 'core'`). Tüm opsiyonel tamam → otomatik detailed + `optionalCompletedAt` + 14 gün kilit. Retake: `{ retakeAt }` + `analysisStage: 'core'`; leftover detailed stage kilitte kalmaz — [health-test-retake-lock.md](health-test-retake-lock.md). Hub + panel `HealthScoreCard`. Engine: `src/services/healthScoreAnalysis.ts`. AI: `POST /api/ai-health-analysis`; fallback deterministic.

**Lab uploads:** Private bucket `health-lab-results`, path `{userId}/{ts}-{rand}.{ext}`, via `uploadHealthLabResult`. Profil paneli yalnız `bloodWorkUploadIntent === 'yes'`. Personel sağlık sayfası dosya varsa salt okunur gösterir. MOBILE DIFF: yeni yükleme galeri görseli (`pickLabFile`); web’den PDF açılır.
