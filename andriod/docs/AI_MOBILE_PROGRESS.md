...
> **Son güncelleme:** 2026-08-28 — Android bildirim kanalı erken kayıt + OEM pil uyarısı; iOS `audio` background mode kaldırıldı  
...
> **Platform:** Android — build yalnız `npm run build:play:android` (onaysız eas yok)

## 2026-08-28 Bildirim kanalı + OEM pil + iOS audio

| Madde | Durum |
|-------|--------|
| Android `yeniform-alerts-v3` auth’tan önce `app/_layout` kaydı | ✅ |
| `ensureNotificationChannel` hata → retry (`channelReady = null`) | ✅ |
| Staff `StaffPushBootstrap` + üye/staff realtime chat banner | ✅ |
| OEM “arka planda otomatik kapat” bir kez Alert → uygulama ayarları | ✅ `expo-intent-launcher` |
| iOS `UIBackgroundModes`: `audio` kaldırıldı; `voip` + `remote-notification` | ✅ |

## 2026-08-27 Android personel + danışan — iOS parity

iOS’ta kalan panel güncellemeleri Android’e taşındı. App Store 3.1.3 sapmaları **kopyalanmadı** (ödeme sayfası, Stripe portal, web checkout CTA durur).

| Madde | Durum |
|-------|--------|
| Canlı sohbet: `liveChatMerge` + thread-filtre subscribe + optimistic send | ✅ üye + personel client/admin/collab |
| Sohbet açılınca ilgili in-app bildirim okundu (`markRelatedChatNotificationsRead`) | ✅ |
| Push: cihaz token tek sahibi, logout wipe, `userId` kapısı | ✅ |
| Randevu bildirimi → `/(staff)?focus=appointments&sessionId=` + satır vurgu | ✅ |
| `from=` deep-link + rol remap (`panelRouteRemap`, `/(staff)` path) | ✅ |
| Kalori: `ChatKeyboardLayout` (KeyboardStickyView inverted list örtmesin) | ✅ |
| Inbox ensure-once + sağlık fotoğrafı + analiz toast + AuthBackButton onLight | ✅ |
| Çift “Yeni mesaj” fallback (client/collab) kaldırıldı | ✅ |
| Görüşme yatay: “Telefonu dik tutun” overlay | ✅ |
| Ödeme / Stripe portal / `canOfferWebPurchase` | ✅ durur (Play) |
| Onaylı yeni AAB | 🔄 sen `npm run build:play:android` |

## 2026-08-27 sağlık testi retake kilit (web)

Web Adsız `ca4733bd`. Spec: [`docs/mobile/domains/health-test-retake-lock.md`](./mobile/domains/health-test-retake-lock.md)

| Madde | Durum |
|-------|--------|
| Docs/skill/F04/hub/core/section LOCK güncellendi | ✅ |
| `getHealthTestLockState` + `retakeAt` + retake `analysisStage: 'core'` | ✅ web `healthTestLock.js` / `HealthTestPage.handleRetake` |

## 2026-08-22 iOS + Android ayrımı + pazar hazırlık

| Madde | Durum |
|-------|--------|
| `eas.json` yalnız Android profilleri (`play-store` AAB) | ✅ |
| Store docs: iOS dosyaları bu klasörden çıkarıldı | ✅ |
| Logout → yeniden giriş (AsyncStorage race / staff) | ✅ |
| Profil düzenle: nested Modal (şehir/tarih) | ✅ web kilit: e-posta / telefon-sonra / cinsiyet-sonra |
| Koç sepeti `durationUnit` sn/dk | ✅ web CartEntryCard |
| Realtime toast: program + randevu | ✅ |
| Play: ML Kit PORTRAIT override + cutout ALWAYS | ✅ edge-to-edge opt-out yok (API 36) |
| Form loading overlay (giriş/kayıt/kalori) | ✅ |
| Onaylı yeni AAB | 🔄 sen `npm run build:play:android` |

## 2026-08-22 iOS ödeme + program bildirim + video

| Madde | Durum |
|-------|--------|
| iOS drawer / profil / dashboard Ödeme Yönetimi yok | ✅ Portal ileride |
| `profile/payments` iOS → profil redirect | ✅ Android durur |
| Tip 1: `entry.start` öğün/antrenman OS zili (`yf-prog-*`) | ✅ generic meal/workout replace |
| Tip 2: 20:00 `habit_no_activity` (hiç işaret yok) | ✅ 22:00–08:00 sessiz |
| Randevu Katıl 30s tick + join penceresi açıkken upcoming | ✅ üye + personel |
| Daily URL yok log + WebView izin toast | ✅ |
| `isJoinWindowOpen` koç 10/20 | ✅ |
| Onaylı yeni AAB / IPA | 🔄 iOS 1.0.2 store + TestFlight |

## 2026-08-22 Oturum 401 + diyet video

| Madde | Durum |
|-------|--------|
| Programlarım beslenme satırında thumbnail / play / signed URL yok | ✅ web `ProgramsPage` parity |
| AppState `startAutoRefresh` / `stopAutoRefresh` (ön plan) | ✅ |
| `getApiAuthHeaders` süresi dolan JWT’yi `refreshSession` ile yeniler | ✅ handoff yolu dokunulmadı |
| `postJson` 401: “Oturumunuzun süresi doldu…” | ✅ |
| Onaylı yeni AAB / IPA | 🔄 iOS 1.0.2 store + TestFlight |

## 2026-08-22 iOS satın alma dili

| Madde | Durum |
|-------|--------|
| Randevu / kalori kilit: iOS’ta Paketleri gör + “satın alın” yok | ✅ |
| Kayıt hero: iOS yükseltme vaadi yok (`iosOnboardingSub`) | ✅ |
| `habit_upsell` iOS’ta kurulmaz; tap dashboard | ✅ |
| Galeri gerekçesi kalori fotoğraflarını da söyler | ✅ |
| TestFlight IPA | 🔄 1.0.2 `build:ios:store` + submit |
