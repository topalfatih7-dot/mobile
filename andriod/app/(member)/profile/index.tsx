...
export default function ProfileScreen() {
  ...
  const { email, logout, loggingOut, registeredMember, refreshAuth } = useAuth();
  const member = useMember();
  const { myPrograms, staffById, isFreeTrialExpired, refreshData } = useData();
  const { updateProfile, updateSettings, updateHealthTestPartial } = useActions();
  ...
  const showLabPanel = memberOptedInToHealthLab(healthTest);

  const handleLabFilesChange = useCallback(
    async (nextFiles: { path: string; name: string; contentType: string }[]) => {
      await updateHealthTestPartial(patchHealthTestLabFiles(healthTest, nextFiles));
    },
    [healthTest, updateHealthTestPartial],
  );
    ...
    <MeshBackground style={styles.root}>
      <ScrollView
        ...
        {member && showLabPanel ? (
          <HealthLabFilesPanel
            canEdit
            files={collectHealthLabFiles(healthTest, String(member.id))}
            memberId={String(member.id)}
                ...
                <Text style={styles.expireWarn}>
                  {selectedRemainingDays != null && selectedRemainingDays <= 0
                    ...
                    : offerWebPurchase
                      ...
                      : 'Bu paketin süresi yakında doluyor.'}
                </Text>
              )}
            </View>
          ) : null}
      ...
      <Modal animationType="slide" transparent visible={editOpen}>
        <View style={styles.modalBackdrop}>
          <View
            ...
            <FormKeyboardScroll
              contentContainerStyle={{ gap: spacing.sm, paddingBottom: 8 }}
...
const styles = StyleSheet.create({
  ...
  experts: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, padding: spacing.md, paddingTop: 0 },
  expertCard: {
    flexGrow: 1,
    flexBasis: 96,
    minWidth: 96,
