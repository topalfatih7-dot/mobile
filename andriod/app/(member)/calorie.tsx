...
export default function CalorieScreen() {
  ...
  const send = async () => {
    ...
    if (!result.ok) {
      setMessages((m) => [
        ...m,
        { id: `a-${Date.now()}`, role: 'assistant', text: result.error },
      ]);
      setBusy(false);
      return;
    }
  ...
  const runVision = async (useCamera: boolean) => {
    ...
    if (!analysis.ok) {
      setMessages((m) => [
        ...m,
        { id: `a-${Date.now()}`, role: 'assistant', text: analysis.error },
      ]);
      setBusy(false);
      return;
    }
  ...
  if (isUnpaidMember) {
    return (
      <MeshBackground style={styles.root}>
        <View style={[styles.lock, { paddingTop: spacing.sm }]}>
          <UnpaidMemberGate
          ...
          <View style={styles.lockCard}>
            ...
            <Text style={styles.lockDesc}>
              {offerWebPurchase
                ...
                : MEMBERSHIP_CANCEL_COPY.iosCalorieLocked}
            </Text>
            {offerWebPurchase ? (
              <Button
                label="Paketleri gör"
      ...
      <View style={styles.flex}>
        <View style={styles.header}>
          ...
          <Pressable
            ...
            style={[styles.camBtn, !canPhoto && styles.camLocked, busy && { opacity: 0.55 }]}>
            <LinearGradient
              colors={
                canPhoto
                  ? [colors.brand[500], colors.brand[700]]
                  : [colors.cream[100], colors.cream[200]]
              }
              end={{ x: 1, y: 1 }}
              start={{ x: 0, y: 0 }}
              style={styles.camBtnInner}>
              <Ionicons
                color={canPhoto ? colors.white : colors.cream[300]}
        ...
        <ChatKeyboardLayout
          list={
            ...
            if (item.role === 'user') {
              return (
                <Animated.View
                  entering={enteringNative(FadeInUp.duration(200))}
            ...
            return (
              <Animated.View entering={enteringNative(FadeInUp.duration(200))} style={styles.aiRow}>
          ...
          composer={
            ...
            <Pressable
              ...
              {busy ? (
                <BrandLoader mark={false} size="xs" tone="onDark" />
              ) : (
