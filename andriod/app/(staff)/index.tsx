...
export default function StaffOverview() {
  ...
  const appointmentsY = useRef(0);
  const params = useLocalSearchParams<{
    focus?: string | string[];
    memberId?: string | string[];
    sessionId?: string | string[];
  }>();
  const focusAppointments = String(
    Array.isArray(params.focus) ? params.focus[0] : params.focus || '',
  ) === 'appointments';
  const highlightSessionId = String(
  ...
  const programHref = (
    isCoach ? '/(staff)/programs' : isDietitian ? '/(staff)/lists' : '/(staff)/clients'
  ) as Href;

  const scrollToAppointments = useCallback(() => {
    scrollRef.current?.scrollTo({
      y: Math.max(0, appointmentsY.current - 8),
      animated: true,
    });
  }, []);
  ...
  useEffect(() => {
    if (!focusAppointments) return;
    const t = setTimeout(() => scrollToAppointments(), 350);
    ...
    <PanelScaffold scrollRef={scrollRef} subtitle={subtitle} title={`Merhaba, ${firstName}`}>
      {loading && clients.length === 0 ? (
        ...
        <>
          ...
          <FadeIn delay={40}>
            <View style={[styles.kpiRow, isCompact && styles.kpiRowWrap]}>
              <Pressable
                ...
                <Text numberOfLines={2} style={styles.kpiSub}>
                  Aktif ücretli üye
                </Text>
              </Pressable>
              <Pressable
                accessibilityLabel="Bu hafta randevular"
                accessibilityRole="button"
              ...
              <Pressable
                accessibilityLabel="Oluşturulan programlar"
                accessibilityRole="button"
                onPress={() => router.push(programHref)}
                style={({ pressed }) => [
                  styles.kpi,
                  isCompact && styles.kpiCompact,
                  pressed && styles.kpiPressed,
          ...
          <View
            onLayout={(e) => {
              appointmentsY.current = e.nativeEvent.layout.y;
            }}>
              ...
              <View style={[styles.card, styles.pendingCard]}>
                ...
                {pendingAppointments.slice(0, 8).map((a) => (
                  <View
                    ...
                    style={[
                      styles.queueBlock,
                      highlightSessionId === a.id && styles.sessionHighlight,
                    ]}>
            ...
            <View style={styles.card}>
              ...
              {appointments.length === 0 ? (
                ...
                appointments.slice(0, 8).map((a, i) => {
                  ...
                  const join = canJoinSession(a, sessionType, now);
                  return (
