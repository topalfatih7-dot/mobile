...
import { clearDevicePushToken } from '@/services/push';
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const logoutOverlayTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
    ...
    async (opts: {
      ...
      try {
        ...
        if (!next) {
          ...
          next = (await hydrateAuth(result.session)) || (await hydrateAuth());
        }
        setAuth(next);
        ...
        return { success: true as const, route: routeForHydrated(next), role: next.role };
      } catch {
        return { success: false as const, error: 'Oturum açılamadı. Lütfen tekrar deneyin.' };
      } finally {
  ...
  const logout = useCallback(async () => {
    ...
    setLoggingOut(true);
    if (logoutOverlayTimerRef.current) {
      clearTimeout(logoutOverlayTimerRef.current);
      logoutOverlayTimerRef.current = null;
    }
    loginLockRef.current = true;
    const uid = authRef.current?.userId ? String(authRef.current.userId) : null;
    try {
      ...
      // AsyncStorage signOut kuyruğu bitmeden yeni setSession yazılırsa
      // oturum silinir → "Oturum açılamadı" / staff giriş kırılır.
      await new Promise((r) => setTimeout(r, 400));
    } finally {
      setStaffOverride(null);
      setAuth(null);
      loginEpochRef.current = ++authEventGenRef.current;
      loginLockRef.current = false;
      // Overlay, caller’ın replace/toast’unu örtsün diye bir kare daha kalsın.
      logoutOverlayTimerRef.current = setTimeout(() => {
        logoutOverlayTimerRef.current = null;
  ...
  useEffect(
    () => () => {
      if (logoutOverlayTimerRef.current) clearTimeout(logoutOverlayTimerRef.current);
    },
    [],
  );
  ...
  return (
    <AuthContext.Provider value={value}>
      ...
      {loggingOut ? <LoadingScreen label="Çıkış yapılıyor…" overlay /> : null}
