...
type DailyCallObject = {
  ...
  cycleCamera?: () => Promise<{ device: { facingMode?: CameraFacing } | null }>;
  getCameraFacingMode?: () => Promise<CameraFacing | null>;
};
...
function parseFacing(value: unknown): CameraFacing | null {
  return value === 'user' || value === 'environment' ? value : null;
}

async function syncFacingMode(call: DailyCallObject, desired: CameraFacing) {
  if (desired === 'user') return;
  if (typeof call.getCameraFacingMode === 'function') {
    try {
      const current = parseFacing(await call.getCameraFacingMode());
      if (current === desired) return;
    } catch {
      /* cycle anyway */
    }
  }
  if (typeof call.cycleCamera !== 'function') return;
  try {
...
export function useDailyCall() {
  ...
  const cyclingCameraRef = useRef(false);
  ...
  const join = useCallback(
    async (opts: {
      ...
      try {
        ...
        setPhase('joined');
        refreshParticipants();
        await syncFacingMode(call, facingModeRef.current);
        return { ok: true as const };
      } catch (e: unknown) {
        const err = e as { errorMsg?: string; message?: string };
  ...
  const cycleCamera = useCallback(async () => {
    const call = callRef.current;
    if (!call || !mediaState.camOn || cyclingCameraRef.current) return;
    if (typeof call.cycleCamera !== 'function') {
      setError('Kamera yönü bu cihazda değiştirilemiyor.');
      return;
    }
    cyclingCameraRef.current = true;
    try {
      const result = await call.cycleCamera();
      const next =
        parseFacing(result?.device?.facingMode) ||
        (facingModeRef.current === 'user' ? 'environment' : 'user');
      setFacingMode(next);
      facingModeRef.current = next;
      refreshParticipants();
    } catch {
      setError('Kamera yönü değiştirilemedi.');
    } finally {
      cyclingCameraRef.current = false;
    }
  }, [mediaState.camOn, refreshParticipants]);
