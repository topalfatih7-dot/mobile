import { getJoinWindowMinutes } from '@/services/videoCallSession';
...
/** Sektöre göre katılma penceresi — `getJoinWindowMinutes` ile aynı. */
export function isJoinWindowOpen(
  ...
  const open = start - before * 60_000;
  const close = start + after * 60_000;
  const t = now.getTime();
