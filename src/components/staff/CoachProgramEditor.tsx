...
/**
 * Web CoachProgramEditor RN uyarlaması — 3 adım: Süre → Günler → Gönder.
 * Gönder adımı CoachProgramSendModal açar; create/update çağıran taraf yapar.
 */
export function CoachProgramEditor({
  ...
  const addToCart = (ex: Record<string, unknown>) => {
    ...
    if (cartExerciseIds.has(String(ex.id))) {
      ...
      {step === 3 && !sendOpen ? (
        <View style={styles.block}>
          <LinearGradient
            ...
            <Text style={styles.heroEyebrow}>Hazır</Text>
      ...
      <Modal
        ...
        <Pressable onPress={() => setCartSheetOpen(false)} style={styles.sheetBackdrop}>
          <Pressable
              ...
              {activeCart.length === 0 ? (
                ...
                activeCart.map((entry, idx) => (
                  <View key={entry.id} style={styles.cartCard}>
                    <Pressable
                      onPress={() =>
                        (entry.videoUrl || entry.videoPending) &&
                        setActiveExercise({
                    ...
                    <View style={styles.exInfo}>
                      ...
                      <View style={styles.amountRow}>
                        <View style={styles.miniSeg}>
                          {(
                            ...
                            <Pressable
                              key={m.id}
                              onPress={() =>
                                updateCartItem(entry.id, { amountType: m.id })
                              }
                              style={[
                                styles.miniSegItem,
                                entry.amountType === m.id && styles.miniSegOn,
                              ]}>
                              <Text
                                style={[
                                  styles.miniSegText,
                                  entry.amountType === m.id && styles.miniSegTextOn,
                                ]}>
                                {m.label}
                              </Text>
                            </Pressable>
                        ...
                        <View style={styles.amountCtrl}>
                          <Pressable
                            onPress={() =>
                              updateCartItem(entry.id, {
                                amount: Math.max(1, (Number(entry.amount) || 1) - 1),
                              })
                            }
                            style={styles.amountBtn}>
                            <Ionicons color={colors.cream[800]} name="remove" size={14} />
                          </Pressable>
                          <Text style={styles.amountVal}>{entry.amount}</Text>
                          <Pressable
                            onPress={() =>
                              updateCartItem(entry.id, {
                                amount: (Number(entry.amount) || 0) + 1,
                              })
                            }
                            style={styles.amountBtn}>
                            <Ionicons color={colors.cream[800]} name="add" size={14} />
                          </Pressable>
                        </View>
                      </View>
                      <TextInput
                        onChangeText={(note) => updateCartItem(entry.id, { note })}
                    ...
                    <View style={styles.cartActions}>
                      <Pressable
                        disabled={idx === 0}
                        onPress={() => moveCartItem(entry.id, -1)}
                        style={styles.iconBtn}>
                        <Ionicons color={colors.cream[800]} name="chevron-up" size={16} />
                      </Pressable>
                      <Pressable
                        disabled={idx === activeCart.length - 1}
                        onPress={() => moveCartItem(entry.id, 1)}
                        style={styles.iconBtn}>
                        <Ionicons
                      ...
                      <Pressable
                        ...
                        style={styles.iconBtn}>
                        <Ionicons color={colors.warm[500]} name="trash" size={16} />
                      </Pressable>
                    </View>
                  </View>
                ))
            ...
            <Button label="Kapat" onPress={() => setCartSheetOpen(false)} size="md" />
          </Pressable>
        </Pressable>
      </Modal>
