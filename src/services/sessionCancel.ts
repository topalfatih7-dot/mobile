...
export async function requestCancelSessionApi(opts: {
  ...
  if (isUiOnly()) return { ok: false, error: 'İptal işlemi demo modda kapalı.' };
...
export async function respondCancelSessionApi(opts: {
  ...
  if (isUiOnly()) return { ok: false, error: 'İşlem demo modda kapalı.' };
...
export async function rescheduleSessionApi(opts: {
  ...
  | CancelFail
> {
  if (isUiOnly()) return { ok: false, error: 'Yeniden planlama demo modda kapalı.' };
