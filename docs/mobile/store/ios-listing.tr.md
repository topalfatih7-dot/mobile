# iOS App Store — TR listing şablonu

> Placeholder — sen doldurursun. Bundle: `com.yeniform.app`

## Ad

Yeni Form

## Alt başlık (30 karakter)

```
[PLACEHOLDER — örn. Koç + diyetisyen + sağlık]
```

## Açıklama

```
[PLACEHOLDER — uzun açıklama]

Yeni Form ile:
• …
• …
• …

Destek: …
```

## Anahtar kelimeler (100 karakter, virgülle)

```
yeni form,diyet,koc,saglik,kalori,antrenman,[PLACEHOLDER]
```

## Destek URL

https://www.yeniform.com

## Privacy Policy URL

https://www.yeniform.com/legal/gizlilik-politikasi

## Terms URL

https://www.yeniform.com/legal/uyelik-ve-abonelik-sozlesmesi

## Kategori

Health & Fitness

## Yaş

12+ (veya mağaza uygunluğu)
