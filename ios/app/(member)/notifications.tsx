...
import { displayMemberNotificationTitle } from '@/services/memberNotifications';
...
export default function NotificationsScreen() {
  ...
  const openNotif = async (n: Notif) => {
    ...
    if (n.type === 'call-join') {
      const sid = String(n.sessionId || '').trim();
      const st = String(n.sessionType || n.staffRole || '').trim();
      if (sid && (st === 'coach' || st === 'dietitian' || st === 'doctor')) {
        router.push(`/(member)/call/${st}/${sid}` as Href);
        return;
      }
      router.push('/(member)/schedule' as Href);
      return;
              ...
              <Pressable
                ...
                <View style={{ flex: 1 }}>
                  <Text style={styles.cardTitle}>
                    {displayMemberNotificationTitle(n.title)}
                  </Text>
                  {n.message || n.text ? (
