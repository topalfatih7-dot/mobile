...
export default function MessagesIndex() {
  ...
  const reload = useCallback(async (mode: 'ensure' | 'fetch' = 'fetch') => {
    if (!member?.id) {
      setThreads([]);
      setBusy(false);
      return;
    ...
    try {
      const list =
        mode === 'ensure' || !ensuredRef.current
          ? await ensureMemberChatThreads(
              contacts,
              String(member.id),
              String(member.name || 'Üye'),
            )
          : await fetchMemberChatThreads(String(member.id));
  ...
  useEffect(() => subscribeBump(() => void reload('fetch')), [subscribeBump, reload]);

  // Thread’den dönüşte rozet tazele (Expo Router stack — web navigate('/messages') eşleniği)
  useFocusEffect(
    useCallback(() => {
      setBusy(true);
      void reload(ensuredRef.current ? 'fetch' : 'ensure');
    }, [reload]),
  );
    ...
    return (
      <MeshBackground style={styles.root}>
        <ScrollView
          ...
          <UnpaidMemberGate
            description="Geçmiş sohbetleriniz saklanır. Yeni mesaj göndermek bu pakette yer almaz."
