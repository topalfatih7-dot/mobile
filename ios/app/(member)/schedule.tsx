...
export default function ScheduleScreen() {
  ...
  useIntervalWhileActive(() => setNowMs(Date.now()), 30_000, true, {
    requireFocus: true,
  });
  ...
  const lockedLead = offerWebPurchase
    ? tabMeta.lockedDescription
    : MEMBERSHIP_CANCEL_COPY.iosScheduleLockedLead[tab];
  const lockedDescription = offerWebPurchase
    ? tabMeta.lockedDescription
    : MEMBERSHIP_CANCEL_COPY.iosScheduleLocked[tab];
  ...
  const filteredSessions = useMemo(() => {
    ...
    if (listFilter === 'past') {
      return sessions
        .filter((s) => s.status === 'completed' || (s.date && new Date(s.date).getTime() < nowMs))
        .reverse();
    }
    return sessions.filter((s) => {
      if (!UPCOMING_STATUSES.includes(s.status || 'scheduled') || !s.date) return false;
      if (new Date(s.date).getTime() >= nowMs) return true;
      return canJoinSession(s, tab, new Date(nowMs)).ok;
    });
  }, [sessions, listFilter, nowMs, tab]);
    ...
    return (
      <MeshBackground style={styles.root}>
        <ScrollView
          ...
          <UnpaidMemberGate
            description="Bu sayfayı gezebilirsiniz. Koç, diyetisyen ve doktor randevusu bu pakette yer almaz."
      ...
      <ScrollView
        ...
        <FadeIn>
          <View style={styles.heroCard}>
            ...
            <Animated.View
              entering={enteringNative(ReFadeIn.duration(150))}
              key={tab}
        ...
        {showLocked ? (
          <FadeIn delay={80}>
            <View style={styles.locked}>
              ...
              <Text style={styles.lockedDesc}>{lockedLead}</Text>
              {offerWebPurchase ? (
                <Button
                  label="Paketleri gör"
          ...
          <>
            ...
            {filteredSessions.length === 0 ? (
              ...
              filteredSessions.map((s, i) => {
                const isPast = s.date ? new Date(s.date).getTime() < nowMs : false;
                ...
                const showBlocked =
                  ['scheduled', 'rescheduled'].includes(status) && !isPast && !withinNotice;
                const joinCheck = canJoinSession(s, tab, new Date(nowMs));
