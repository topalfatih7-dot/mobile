            ...
            ? groupBySchedule(entries, p).map((g) => (
                <View key={g.key} style={styles.group}>
                  ...
                  {g.items.map((e: Record<string, unknown>) => {
                    ...
                    const setsLabel =
                      e.sets != null && e.sets !== ''
                        ...
                        : e.amount
                          ...
                          : '';
                    const rowKey = String(e.id || `${g.key}-${title}`);
                    const body = (
                      <>
                        {isNutrition ? (
                          <View style={styles.mealIcon}>
                            <Ionicons color={colors.sage[600]} name="nutrition" size={22} />
                          </View>
                        ) : (
                    ...
                    if (isNutrition) {
                      return (
                        <View key={rowKey} style={styles.entry}>
                          {body}
                        </View>
                      );
                    }
    ...
    return (
      <MeshBackground style={styles.root}>
        <ScrollView
          ...
          <UnpaidMemberGate
            description="Bu sayfayı gezebilirsiniz. Size özel antrenman ve beslenme programları bu pakette yer almaz."
