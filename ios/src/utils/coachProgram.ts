...
export const DEFAULT_CART_SETS = 3;
...
export function cartEntryAmountLabel(e: {
  amountType?: string;
  amount?: number;
  durationUnit?: string;
}) {
  return e.amountType === 'duration'
    ? `${e.amount ?? 0} ${e.durationUnit || 'sn'}`
    : `${e.amount ?? 0} tekrar`;
}

export function cartEntrySummary(e: {
  ...
  const sets =
    e.sets != null && e.sets !== '' ? `${e.sets} set × ${cartEntryAmountLabel(e)}` : cartEntryAmountLabel(e);
  return `${sets}${e.note ? ` · ${e.note}` : ''}`;
}

export function entryToDisplayText(e: Record<string, unknown>) {
  const day =
    e.day != null && e.day !== '' ? `${weekdayShortLabel(e.day as number)} · ` : '';
  return `${day}${e.exerciseName} · ${cartEntrySummary(e)}`;
}
...
function normalizeCartEntry(
  ...
  return {
    ...
    sets: Number(e.sets ?? DEFAULT_CART_SETS) || DEFAULT_CART_SETS,
    note: e.note || '',
    start,
    end,
    order: i,
