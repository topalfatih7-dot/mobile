...
/**
 * Üye/personel web path’leri çakışmasın diye personel `/staff/…` altındadır.
 * Eski çakışan path (`/messages/:id`) personel href’ine çevrilir.
 */
export function staffHrefForPath(pathname: string): string {
  ...
  if (a === 'staff') return path === '/staff' ? '/staff' : path;
