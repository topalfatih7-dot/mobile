...
export function ExerciseDetailModal({
  ...
  const loadUrl = useCallback(async () => {
    ...
    try {
      ...
      if (gen === loadGen.current) setLoadingVideo(false);
    }
  ...
  const exerciseBadges = useMemo(() => {
    type Tone = 'brand' | 'sage' | 'warm' | 'cream' | 'mint';
    const items: { label: string; tone: Tone }[] = [];
    const seen = new Set<string>();
    const add = (label: string, tone: Tone) => {
      const key = label.trim().toLocaleLowerCase('tr');
      if (!key || seen.has(key)) return;
      seen.add(key);
      items.push({ label: label.trim(), tone });
    };
    if (exercise?.category) add(String(exercise.category), 'brand');
    if (exercise?.equipment) add(String(exercise.equipment), 'sage');
    if (exercise?.difficulty) {
      add(DIFFICULTY_LABELS[String(exercise.difficulty)] || String(exercise.difficulty), 'cream');
    }
    formatExerciseLocations(exercise?.locations).forEach((loc) => add(loc, 'warm'));
    if (exercise?.requiresMachine) add('Makinalı', 'cream');
    if (exercise?.targetMuscle || exercise?.bodyPart) {
      add(String(exercise.targetMuscle || exercise.bodyPart), 'mint');
    }
    return items;
  }, [exercise]);
        ...
        <View style={styles.card}>
          <ScrollView
            ...
            <VideoPlayer
              ...
              title={name}
              url={visible && canPlay && !videoPending ? playUrl : null}
              videoPending={videoPending && canPlay}
              videoRef={videoRef}
            />

            <View style={styles.badges}>
              {exerciseBadges.map((b) => (
                <Badge key={`${b.tone}-${b.label}`} label={b.label} tone={b.tone} />
              ))}
            </View>
