...
export type ChatRealtimeChange = {
  eventType: string;
  table: 'chat_threads' | 'chat_messages';
  newRow: Record<string, unknown> | null;
};

export function parseChatMessageRow(row: Record<string, unknown>): ChatMessage {
  return rowToChatMessage(row);
}

export function parseChatThreadRow(row: Record<string, unknown>): ChatThread {
  return rowToChatThread(row);
}

/** Inbox / badge: threads only (unread lives on chat_threads.data). */
...
/** Staff inbox / badge: threads only. */
...
/** Open thread: filtered message INSERT/UPDATE. */
export function subscribeChatThreadMessages(
  threadId: string,
  listener: (change: ChatRealtimeChange) => void,
): () => void {
  if (!threadId) return () => {};
  const sb = requireSupabase();
  channelSequence += 1;
  const channel = sb
    .channel(`chat-msgs-${threadId}-${channelSequence}`)
    .on(
      'postgres_changes',
      {
        event: '*',
...
export async function sendChatMessage(
  ...
  { success: true; message?: ChatMessage } | { success: false; error: string }
> {
  ...
  // DB trigger `tg_chat_message_touch_thread` (20260825 migration) zaten
  // INSERT sonrası last_message_at, lastPreview ve staffUnread'i atomik günceller.
  // Manuel UPDATE kaldırıldı — çift artırma (double increment) hatası önlenir.
  const preview =
    trimmed.length > 120 ? `${trimmed.slice(0, 119)}…` : trimmed;
  let staffIdForNotify = '';
  let threadMemberIdForNotify = memberId;
  let memberNameForNotify = '';
  try {
    const { data: row } = await sb
      .from('chat_threads')
      .select('member_id, staff_id, data')
      .eq('id', threadId)
      .maybeSingle();
    if (row) {
      const prev = (row.data as Record<string, unknown>) || {};
      staffIdForNotify = String((row as { staff_id?: string }).staff_id || '');
      threadMemberIdForNotify = String(
        (row as { member_id?: string }).member_id || memberId,
      );
      memberNameForNotify = String(prev.memberName || '');
    }
  } catch {
    /* ignore thread read for notify lookup */
  }

  if (staffIdForNotify) {
    void notifyStaffChatMessage({
      staffId: staffIdForNotify,
      preview,
      threadId,
      memberId: threadMemberIdForNotify,
  ...
  return {
    success: true,
    message: inserted
      ? rowToChatMessage(inserted as Record<string, unknown>)
      : undefined,
  };
}
...
> {
  ...
  // DB trigger `tg_chat_message_touch_thread` (20260825 migration) zaten
  // INSERT sonrası last_message_at, lastPreview ve memberUnread'i atomik günceller.
  // Manuel UPDATE kaldırıldı — çift artırma (double increment) hatası önlenir.
