...
const LAB_MAX_BYTES = 8 * 1024 * 1024;
const LAB_ALLOWED_EXT = ['pdf', 'jpg', 'jpeg', 'png', 'webp'];
const LAB_SIGNED_TTL_SEC = 900;
...
export async function uploadHealthLabResult(opts: {
  ...
  try {
    if (!opts.userId) return { ok: false, error: 'Oturum gerekli' };
    const ext = (opts.fileName?.split('.').pop() || 'jpg').toLowerCase();
    if (!LAB_ALLOWED_EXT.includes(ext)) {
      return { ok: false, error: 'Yalnızca PDF veya görsel yükleyebilirsiniz' };
    }
    const sb = requireSupabase();
    const path = `${opts.userId}/${Date.now()}-${randomSuffix()}.${ext}`;
    const res = await fetch(opts.uri);
    const buf = await res.arrayBuffer();
    if (buf.byteLength > LAB_MAX_BYTES) {
    ...
    if (error) return { ok: false, error: error.message };
    const signed = await sb.storage.from(LAB_BUCKET).createSignedUrl(path, LAB_SIGNED_TTL_SEC);
    return {
      ok: true,
      meta: {
        path,
...
export async function getHealthLabResultUrl(
  path: string,
  memberId: string,
): Promise<{ ok: true; url: string } | { ok: false; error: string }> {
  const { isHealthLabStoragePath } = await import('@/utils/healthLabFiles');
  if (!isHealthLabStoragePath(path, memberId)) return { ok: false, error: 'Geçersiz dosya' };
  try {
    const sb = requireSupabase();
    const { data, error } = await sb.storage
      .from(LAB_BUCKET)
      .createSignedUrl(path, LAB_SIGNED_TTL_SEC);
    if (error || !data?.signedUrl) {
      return { ok: false, error: error?.message || 'Bağlantı oluşturulamadı' };
    }
...
export async function removeHealthLabResult(
  path: string,
  userId: string,
): Promise<{ ok: true } | { ok: false; error: string }> {
  const { isHealthLabStoragePath } = await import('@/utils/healthLabFiles');
  if (!isHealthLabStoragePath(path, userId)) return { ok: false, error: 'Geçersiz dosya' };
  try {
    const sb = requireSupabase();
    const { error } = await sb.storage.from(LAB_BUCKET).remove([path]);
    if (error) return { ok: false, error: error.message };
    return { ok: true };
  } catch (e) {
    return { ok: false, error: String((e as Error)?.message || e) };
