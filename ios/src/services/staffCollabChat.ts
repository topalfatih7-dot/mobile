...
import { notifyStaffCollabMessage } from '@/services/staffNotifications';
...
export async function fetchStaffCollabThreadById(
  threadId: string,
  ...
  return rowToStaffCollabThread(data as Record<string, unknown>);
}

export async function fetchStaffCollabThreadByMemberId(
  memberId: string,
): Promise<StaffCollabThread | null> {
...
/** Inbox uses memberId; push may send memberId or collab thread UUID. */
export async function resolveStaffCollabThread(
  routeParam: string,
): Promise<StaffCollabThread | null> {
  const id = String(routeParam || '').trim();
  if (!id) return null;
  const [byMember, byId] = await Promise.all([
    fetchStaffCollabThreadByMemberId(id),
    fetchStaffCollabThreadById(id),
  ]);
  return byMember || byId;
}
...
export async function sendStaffCollabMessage(opts: {
  ...
  const liveData = (liveRow.data as Record<string, unknown>) || {};
  const coachId = String(liveRow.coach_id || opts.thread.coachId || '');
  const dietitianId = String(liveRow.dietitian_id || opts.thread.dietitianId || '');
  const doctorId = String(liveRow.doctor_id || opts.thread.doctorId || '');
  const memberId = String(liveRow.member_id || opts.thread.memberId || '');
  const memberName = String(liveData.memberName || opts.thread.memberName || '');
  const selfId = opts.senderId ? String(opts.senderId) : '';

  const peerIds = new Set<string>();
  if (opts.senderType !== 'coach' && coachId) peerIds.add(coachId);
  if (opts.senderType !== 'dietitian' && dietitianId) peerIds.add(dietitianId);
  if (opts.senderType !== 'doctor' && doctorId) peerIds.add(doctorId);
  await Promise.all(
    [...peerIds]
      .filter((peerId) => peerId && peerId !== selfId)
      .map((peerId) =>
        notifyStaffCollabMessage({
          staffId: peerId,
          preview,
          threadId: opts.thread.id,
          memberId,
          memberName,
          senderRole: opts.senderType,
...
export function parseStaffCollabMessageRow(
  row: Record<string, unknown>,
): StaffCollabMessage {
  return rowToStaffCollabMessage(row);
}

/** Inbox / badge: collab threads only. */
...
export function subscribeStaffCollabMessages(
  threadId: string,
  ...
  collabChannelSeq += 1;
  const channel = client
    .channel(`staff-collab-msgs-${threadId}-${collabChannelSeq}`)
    .on(
      'postgres_changes',
      {
        event: '*',
        schema: 'public',
        table: 'staff_collab_messages',
