 ...
 */
export function isRetakeAfterLockStart(
  retakeAt: unknown,
  lockTs: number | null,
): boolean {
...
export function resolveOptionalCompletedAtTimestamp({
  ...
  const fromAnalysis =
    healthAnalysis?.analysisStage === 'detailed'
...
function unlockedLockState(): HealthTestLockState {
  return {
    locked: false,
    lockedUntil: null,
    daysLeft: 0,
...
/**
 * 14 günlük kilit durumu. Web `src/utils/healthTestLock.js` parity.
