...
Bu klasör **yalnız iOS** projesidir. Android kodu / Play Store build **yok**.

- Store build: `npm run build:ios` veya `npm run build:ios:store` (`eas.json` profil `app-store`, `--platform ios`)
- Preview: `npm run build:preview:ios`
- Submit: `npm run submit:ios`
- **Onaysız `eas build` yok.** Kullanıcı açıkça iOS build/submit demeden başlatma.
- Android build bu klasörden çalıştırılmaz. Play AAB için `../andriod` kullanılır.
...
Expo SDK **56** + Expo Router · Auth/Data/Actions Context · Ödeme: uygulama içi IAP yok; iOS’ta web satın alma CTA yok (App Store 3.1.3(f)).
