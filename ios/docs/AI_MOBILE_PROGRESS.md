...
> **Son güncelleme:** 2026-08-27 — sağlık testi retake kilit (web + mobil kod)  
...
> **Platform:** iOS — build yalnız `npm run build:ios` (onaysız eas yok)
...
## 2026-08-26 iOS klavye overlay

| Madde | Durum |
|-------|--------|
| Mesaj/kalori: klavye composer + son balonları örtüyordu | ✅ `KeyboardDock` (layout padding) |
| Giriş/kayıt: sticky transform formun üstüne biniyordu | ✅ dock + CTA layout; StickyView kaldırıldı |
| Onaylı yeni IPA | 🔄 sen `npm run build:ios` |

## 2026-08-26 iOS UI / UX / form

| Madde | Durum |
|-------|--------|
| Giriş/kayıt sticky: klavye açıkken home-indicator boşluğu | ✅ `KeyboardDock` |
| Kayıt / reset: sistem Alert → `FormErrorModal`; forgot → toast | ✅ lock |
| iOS şifre alanı özel font mermileri | ✅ TextField system font |
| Return/Next, iOS autofill (`username`/`newPassword`) | ✅ |
| Ana ekran / Girişe dön krem zeminde okunmaz beyaz chip | ✅ `AuthBackButton` onLight |
| Kalori: klavyede mesaj listesi input ile yükselir | ✅ `ChatKeyboardLayout` |
| Toast Dynamic Island altına | ✅ `insets.top` |
| Onaylı yeni IPA | 🔄 sen `npm run build:ios` |
...
| Madde | Durum |
|-------|--------|
| `eas.json` yalnız iOS profilleri (`app-store`) | ✅ |
| Store docs: Android dosyaları bu klasörden çıkarıldı | ✅ |
| Logout → yeniden giriş (AsyncStorage race / staff) | ✅ |
| Profil düzenle: nested Modal (şehir/tarih) iOS | ✅ web kilit: e-posta / telefon-sonra / cinsiyet-sonra |
| Koç sepeti `durationUnit` sn/dk | ✅ web CartEntryCard |
| Realtime toast: program + randevu | ✅ |
| `profile/payments` → profil | ✅ |
