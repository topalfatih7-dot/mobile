...
| condition | navigate |
|-----------|----------|
| `type === 'appointment'` | `/(staff)?focus=appointments` (+ `memberId` / `sessionId` query varsa) |
| `type === 'call-join'` | `/(staff)/call/{sessionType}/{sessionId}` (eksikse overview `?focus=appointments`) |
| `type === 'chat' && memberId` | `/(staff)/messages/{memberId}` |
| `type === 'admin-chat'` | `/(staff)/messages/admin/{threadId}` (yoksa `/(staff)/messages/admin`) |
| `type === 'collab' && memberId` | `/(staff)/messages/collab/{memberId}` |

Always `markNotificationRead(n.id)` first. OS push tap uses the same map (`routeFromStaffPushData`, `audience: 'staff'`).

Kaynak: `append_staff_notification` + `POST /api/application-notify` `{ staffId, memberId: staffId, notification }`. Expo gönderen tarafta atılır; alıcı layout ikinci push üretmez.
...
- [ ] Filter labels exact
- [ ] Navigate map: appointment→overview `?focus=appointments`, call-join→call screen, chat→messages/:memberId, admin-chat→admin thread, collab→collab/:memberId
