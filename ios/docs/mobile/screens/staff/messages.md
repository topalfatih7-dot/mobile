...
4. Thread: header (isim + rol · plan) → `ChatCollapsiblePrograms` (coach → workout, dietitian → nutrition; **doctor → gizli**) → **inverted** mesaj listesi (gün ayıracı) → yüksek composer  
   Aynı sohbet UX: danışan thread, collab, admin thread (`toInvertedChatRows`; klavyede son mesaj composer’ın üstünde).  
