...
Uygulamada kayıt **tek adım ücretsiz üyelik** (Stepper yok; Üyelik adımı yok). Ücretli paket: **Android** panel → login’li web `/plans`. **iOS:** satın alma CTA yok. Hero alt: Android yükseltme cümlesi; iOS `iosOnboardingSub` (“panelden yükseltebilirsin” yok). Aşağıdaki web Stepper / plan query davranışı **yalnız web**.

**MOBILE DIFF (2026-08-24):** Üst hero (logo/wordmark bloğu) yok. “Ücretsiz kayıt ol” başlığı + alt metin beyaz form kartının içinde. “Ana ekran” scroll sonunda (krem zeminde okunur chip). Klavye: `FormKeyboardScroll`; **Ücretsiz kayıt ol** (+ sözleşme kutusu) form kartının içinde, klavyeye yapışık dock yok. Odaklı alan klavyenin üstüne kayar. Hatalar `FormErrorModal` (Alert değil).
