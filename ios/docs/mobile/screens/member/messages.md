...
5. Message list: **inverted** `FlatList` (`toInvertedChatRows`) — newest at the bottom; day chips (Bugün / Dün / tarih)  
6. Composer (text-only): taller min height (~52); keyboard open keeps the latest bubble just above the composer (`ChatKeyboardLayout` / `KeyboardDock` — klavye yüksekliği kadar alt padding; overlay transform yok)  
