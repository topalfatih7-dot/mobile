...
| `type === 'call-join'` | `/call/{sessionType}/{sessionId}` (eksikse `/schedule`) |
| `type === 'assignment'` | `/profile` |
| `action === 'water_goal_updated'` | `/dashboard` |
| `type === 'reminder'` ve `action` habit_meal / habit_workout / habit_streak / habit_program_meal / habit_program_workout / habit_no_activity | `/calendar` |
| `type === 'reminder'` ve `action === 'habit_health'` | `/health-test` |
| `type === 'reminder'` ve `action === 'habit_upsell'` | `/profile/payments` — **iOS:** `/dashboard` |
