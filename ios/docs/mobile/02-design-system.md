...
### Danger (semantik hata — mevcut UI hex birleşimi)
| Token | Hex |
|-------|-----|
| danger-50 | `#fef2f2` |
| danger-100 | `#fee2e2` |
...
Tasarım 1.0 = **390×844**. `src/theme/scale.ts` + `useScaledTheme()`.

| Sınıf | Logical width | Örnek cihaz |
| compact | ≤ 360 | nadir / split |
| regular | 361–411 | SE 375, 16e 390, 16 393, 16 Pro 402 |
| large | 412–447 | 16 Plus 430, 16 Pro Max 440 |
| xl | ≥ 448 | landscape, iPad |

- `scale = clamp(width / 390, 0.88, 1.12)`
- Yazı/ikon: `ms` (faktör 0.5); spacing: `ss` (0.65); dikey: `vs`
- Ölçeklenmez: `borderWidth: 1`, hairline, yüzde genişlik
- Dokunma: ölçek sonrası min 44; birincil CTA min 48
- Sistem yazı (Dynamic Type / fontScale): **kapalı** — `allowFontScaling={false}`, `maxFontSizeMultiplier = 1`. Telefon Ayarlar yazı boyutu uygulamayı değiştirmez.
- Metin taşması: `flexShrink: 1`, `minWidth: 0`, `numberOfLines` satır içi
