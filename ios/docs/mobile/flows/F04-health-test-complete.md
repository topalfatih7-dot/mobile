...
7. After lock expires: **Testi Yeniden Çöz** → `healthTest: { retakeAt }` **+** `healthAnalysis.analysisStage = 'core'` → restart core. `retakeAt` eski detailed kilidini kapatır ([health-test-retake-lock.md](../domains/health-test-retake-lock.md)).
